...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def _generate_and_score_completions(
        ...
        indexs = [x["question_index"] for x in inputs]
        answers = [x["answer_name"] for x in inputs]
        prompts_text = [maybe_apply_chat_template(example, self.processing_class)["prompt"] for example in inputs]
        
        images_path = [os.path.join(self.data_dir, x["image_filename"]) for x in inputs]
        images = [Image.open(image_path).convert("RGB") for image_path in images_path]
        ...
        if self.args.use_vllm:
            ...
            inputs_vllm = []
            for prompt_text, image, image_path in zip(prompts_text, images, images_path):
                inputs_vllm.append({
                    "prompt": prompt_text,
            ...
            all_prompts_text = gather_object(inputs_vllm)
            if self.accelerator.is_main_process:
                # Since 'prompts' contains 'num_generations' duplicates, we first take unique prompts, and generate
                # num_generations outputs for each one. This is faster than generating outputs for each duplicate
                # prompt individually.
                ordered_set_of_prompts = list({(d["prompt"], d["image_path"]): {"prompt": d["prompt"], "multi_modal_data": d["multi_modal_data"]} for d in all_prompts_text}.values())
            ...
            with unwrap_model_for_generation(self.model, self.accelerator) as unwrapped_model:
                prompt_completion_ids = unwrapped_model.generate(
                    prompt_ids, attention_mask=prompt_mask, pixel_values=pixel_values, image_grid_thw=image_grid_thw, generation_config=self.generation_config
                )
        ...
        if self.log_completions and self.state.global_step % self.args.logging_steps == 0:
            indexs_to_log = gather_object(indexs)
            ...
            if self.accelerator.is_main_process:
                ...
                if self.args.report_to and "wandb" in self.args.report_to and wandb.run is not None:
                    ...
                    table = {
                        ...
                        "acc_reward": rewards_per_func[:, 0].tolist(),
